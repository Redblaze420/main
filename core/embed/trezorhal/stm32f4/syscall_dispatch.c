...
void syscall_handler(uint32_t *args, uint32_t syscall) {
  switch (syscall) {
    ...
    case SYSCALL_WAIT_RANDOM:
      wait_random();
      break;
    case SYSCALL_RDI_REFRESH_SESSION_DELAY:
    ...
    case SYSCALL_FLASH_AREA_GET_ADDRESS: {
      const flash_area_t *area = (const flash_area_t *)args[0];
      uint32_t offset = args[1];
      uint32_t size = args[2];
      args[0] = (uint32_t)flash_area_get_address(area, offset, size);
    } break;
    case SYSCALL_FLASH_AREA_GET_SIZE: {
      const flash_area_t *area = (const flash_area_t *)args[0];
      args[0] = flash_area_get_size(area);
    } break;
