...
#define MPU_RASR_ATTR_FLASH_CODE (MPU_RASR_C_Msk | MPU_RASR_XN_Msk)
#define MPU_RASR_ATTR_FLASH_DATA (MPU_RASR_C_Msk)
...
#define SET_REGION(region, start, size, mask, attr, access) \
  do {                                                      \
    ...
    MPU->RNR = region;                                      \
    MPU->RBAR = (start) & ~0x1F;                            \
    MPU->RASR = _size | _mask | _attr | _access;            \
  } while (0)
...
static void mpu_init_fixed_regions(void) {
  ...
  SET_REGION( 2, CCMDATARAM_BASE,       SIZE_64B,   0x00, SRAM,       FULL_ACCESS );
...
mpu_mode_t mpu_reconfig(mpu_mode_t mode) {
  ...
  switch (mode) {
    case MPU_MODE_APP:
      ...
      SET_REGION( 6, 0x4002B000,            SIZE_4KB,  0xC0, PERIPH,     PRIV_RW );
    default:
      // All Peripherals (Privileged, Read-Write, Non-Executable)
      SET_REGION( 6, PERIPH_BASE,           SIZE_1GB,  0x00, PERIPH,     PRIV_RW );
  }
  // clang-format on
