...
typedef struct {
  ...
  uint16_t i_limit;
  // Set if the charging is requested
  bool charging;
...
bool pmic_measure(pmic_report_t* report) {
  ...
  // Update VBUS current limit
  if (!npm1300_set_reg(NPM1300_TASKUPDATEILIMSW,
...
int pmic_get_charge_limit(void) {
  pmic_driver_t* drv = &g_pmic_driver;
...
bool pmic_set_charge_limit(int i_charge) {
  pmic_driver_t* drv = &g_pmic_driver;
  ...
  if (i_charge < 32 || i_charge > 180) {
    // Charging current out of range
    // Limit specified in the datasheet (32-800mA)
    // Our battery limit is 180mA
    return false;
  }

  bool charging = drv->charging;

  if (charging) {
    if (!pmic_charge_enable(false)) {
      return false;
    }
  }
  ...
  if (!npm1300_set_reg(NPM1300_BCHGISETMSB, bchg_iset >> 1)) {
    return false;
  }

  if (!npm1300_set_reg(NPM1300_BCHGISETLSB, bchg_iset & 1)) {
    return false;
  }

  drv->i_charge = i_charge;

  if (charging) {
    if (!pmic_charge_enable(true)) {
      return false;
    }
  }
...
bool pmic_charge_enable(bool enable) {
  ...
  if (enable) {
    ok =
        npm1300_set_reg(NPM1300_BCHGENABLESET, NPM1300_BCHGENABLESET_ENABLECHG);
  } else {
    ok = npm1300_set_reg(NPM1300_BCHGENABLECLR,
